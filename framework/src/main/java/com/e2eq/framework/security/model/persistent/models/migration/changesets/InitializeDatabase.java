...
import com.e2eq.framework.security.model.persistent.models.security.*;
import com.e2eq.framework.security.model.persistent.morphia.*;
...
public class InitializeDatabase implements ChangeSetBean {
      ...
      if (config.checkMigration()) {
         try (MorphiaSession session = dataStore.getDataStore().startSession()) {
            try {
               ...
               ensureCounter("accountNumber", 1000);
               Organization org = ensureOrganization("B2BIntegrator.com",
   ...
   public Organization ensureOrganization (String displayName, String refName, @NotNull @Valid DataDomain dataDomain ) {
      ...
      Organization org;
      if (!oOrg.isPresent()) {
       org =  orgRepo.createOrganization("B2BIntegrator.com", SecurityUtils.systemOrgRefName, SecurityUtils.systemDataDomain);
      }
      else {
         org = oOrg.get();
      }
   ...
   public void createInitialRules() {
      ...
      body = new SecurityURIBody.Builder()
       ...
       b = new Rule.Builder()
         ...
         .withEffect(RuleEffect.ALLOW)
         .withFinalRule(true)
         .withOrFilterString("dataDomain.ownerId:system@b2bintegrator.com");
   ...
   public void createInitialUserProfiles() throws CloneNotSupportedException {
      ...
      if (!userProfileRepo.getByUserId("admin@b2bintegrator.com").isPresent()) {
         DataDomain upDataDomain = SecurityUtils.systemDataDomain.clone();
         upDataDomain.setOwnerId("admin@b2bintegrator.com");
         Set<Role> roles = new HashSet<>();
         roles.add(Role.admin);
         UserProfile up = new UserProfile();

         up.setDataDomain(upDataDomain);
         up.setEmail("admin@b2bintegrator.com");
