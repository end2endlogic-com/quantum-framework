...
* *permissions.adoc* (done):
** The "Row Filter Computation (Data Scoping)" section documents how list/query filters are derived — the static variable map (`${pTenantId}` is the only default scope; org/account/segment are opt-in via authored rules), and the dynamic `AccessListResolver` variables (e.g. `${accessibleLocationIds}`) including the ontology-edge-backed `ListQueryRewriter` path. The gateway's "pass the variableMap into planning" fix above is what makes those `${...}` values resolve for `POST /api/query/find`; without it the filter is built tenant-blind exactly as the Current State notes.
