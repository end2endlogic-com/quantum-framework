...
*   `-e, --expires`: Expiration in seconds. Use `-1` for a non-expiring token.
*   `-d, --description`: Optional description for the service token.
*   `--url`: The base URL of the Quantum API (defaults to `http://localhost:8080`).
...
| `auth.provider`
| (required)
| Comma-separated list of provider names (e.g., `custom`, `custom,oidc`). See xref:multiple-auth-providers.adoc[Multiple Auth Providers].
