...
actor App
participant Repo
participant "ValidationInterceptor"
participant "PermissionRuleInterceptor"
participant "AuditInterceptor"
participant "ReferenceInterceptor"
participant "PersistenceAuditEventInterceptor"

App -> Repo: save/update/delete
Repo -> "ValidationInterceptor": prePersist (bean validation, dataDomain default)
"ValidationInterceptor" --> Repo: ok or throw
Repo -> "PermissionRuleInterceptor": prePersist (authorize)
"PermissionRuleInterceptor" --> Repo: ok or throw
Repo -> "AuditInterceptor": prePersist (stamp audit)
Repo -> "ReferenceInterceptor": prePersist (track references)
Repo -> "PersistenceAuditEventInterceptor": prePersist (append event)
Repo --> App: persist or error
@enduml
....
...
@startuml
actor Client
participant "JAX-RS Resource" as API
participant SecurityFilter
participant "ResourceContext"
participant "RuleContext"

Client -> API: HTTP /{area}/{functionalDomain}/{action}/...
API -> SecurityFilter: request
SecurityFilter -> ResourceContext: derive area/domain/action
SecurityFilter -> RuleContext: build SecurityURI (header+body)
note right of RuleContext
  Match rules by wildcard on SecurityURI
  Roles are evaluated as identities
end note
@enduml
....
...
digraph StateGraph {
  ...
  DELIVERED [label="DELIVERED", shape=doublecircle];
  ...
  PENDING -> PROCESSING [label="approve"];
  ...
  SHIPPED -> CANCELLED [label="cancel"];
}
----
...
- `id` is the record identifier used by the REST endpoints and emitted in completion events.
- `refName`, `displayName`, and `dataDomain` are still available for your application-level correlation and multi-tenant scoping, even though they are not task-specific.
...
| `group`
| Optional Morphia reference to the parent `CompletionTaskGroup`
| Client on create, or the `create/{groupId}` endpoint
...
| Server sets `PENDING` on create; application or API updates afterward
...
| `status`
| Coarse aggregate state: `NEW`, `RUNNING`, `COMPLETE`
| Server sets `NEW` on create; application owns later transitions
...
=== What the framework manages vs. what your application manages

The code-level behavior is intentionally small:

- `CompletionTaskGroupRepo.createGroup(...)` always sets `status=NEW` and `createdDate=now`.
- `CompletionTaskRepo.createTask(...)` always sets `status=PENDING` and `createdDate=now`.
- `CompletionTaskRepo.completeTask(...)` sets `status`, `completedDate=now`, and `result`.
- If the task belongs to a group, task completion emits an SSE message for that group.

The framework does not automatically:

- Change a group from `NEW` to `RUNNING`
- Mark a group `COMPLETE` when all child tasks finish
...
state "CompletionTaskGroup" as Group {
  [*] --> NEW
  NEW --> RUNNING : application updates group
  RUNNING --> COMPLETE : application updates group
}

state "CompletionTask" as Task {
  [*] --> PENDING
...
Task --> Group : optional event emission
@enduml
....

Recommended conventions:

- Set a task to `RUNNING` when a worker begins processing it.
- Use `SUCCESS` and `FAILED` as the terminal task states.
- Set a group to `RUNNING` once at least one child task has begun.
- Set a group to `COMPLETE` only when your own aggregation logic decides the batch is finished.
...
| `POST /integration/completionTaskGroup/create`
| Create a new task group
| Server forces `status=NEW` and `createdDate`

| `GET /integration/completionTaskGroup/id/{id}`
| Fetch a group by Mongo `id`
| Inherited from `BaseResource`

| `GET /integration/completionTaskGroup/refName/{refName}`
| Fetch a group by `refName`
| Inherited from `BaseResource`

| `GET /integration/completionTaskGroup/subscribe/{id}`
...
| `POST /integration/completionTask/create/{groupId}`
| Create a task and attempt to attach it to the given group
| Server forces `status=PENDING` and `createdDate`

| `PUT /integration/completionTask/complete/{id}?status=SUCCESS&result=...`
| Update a task status, set `completedDate`, and store `result`
| Despite the method name, the code accepts any `CompletionTask.Status`

| `GET /integration/completionTask/id/{id}`
| Fetch a task by Mongo `id`
| Inherited from `BaseResource`

| `GET /integration/completionTask/refName/{refName}`
| Fetch a task by `refName`
| Inherited from `BaseResource`
|===
...
- The endpoint path requires a `groupId`.
- If the `groupId` does not resolve to an existing group, the current repository implementation still creates the task, but leaves `group` unset.
- The server ignores any client-supplied `status` or `createdDate` during creation and overwrites them.
...
- `completedDate` is always set by the completion endpoint.
- The endpoint name says "complete", but the implementation accepts any value in `CompletionTask.Status`.
...
- Keep `details` short and stable so operators can quickly scan task lists.
- Use `result` for outcome summaries, error messages, or external correlation identifiers.
- Populate `refName` with a meaningful business key so tasks can be found without raw Mongo ids.
- Use `dataDomain` consistently so task and group records stay tenant-scoped.
