...
== Seed packs vs BootstrapPacks

Seed packs and BootstrapPacks solve adjacent problems, but they are not interchangeable.
...
| What they manage
| Deterministic tenant data
| Operational setup steps

| Main representation
| YAML manifest plus JSON/NDJSON datasets
| Java pack definitions plus step handlers

| Best use cases
| Code lists, policies, defaults, baseline reference records
| Calling services, validating prerequisites, external side effects, multi-step orchestration

| Idempotency style
| Natural keys plus dataset checksum tracking
| Handler-driven behavior using apply modes and step policies
|===

Use a seed pack when the desired end state is "these records should exist in the realm."

Examples:

- install baseline code lists
- create default security policies
- ship default surveys or catalog metadata

Use a BootstrapPack when the desired end state is "these setup actions should run in order."

Examples:

- invoke an application service to create a workspace
- call an external registration endpoint
- verify that a downstream dependency has been configured
- publish already-seeded workflow metadata to another platform

In practice, many products use both:

- seed packs establish the durable tenant data model
- BootstrapPacks orchestrate the operational work that sits around that data

See xref:bootstrap-packs.adoc[Bootstrap Packs] for the runtime, SPI, startup, and admin API details.
...
[NOTE]
====
Realm filtering requires `quantum.seed-pack.scopes.enabled=true` in your application properties. When scopes are disabled (the default), all seed packs apply to all realms regardless of scope configuration.
====

=== Include realms (explicit list)
...
The `quantum-system-service` exposes admin-only endpoints to inspect and apply
seed packs per realm (tenant DB). The service opts into the
`com.end2endlogic:quantum-seed-rest` adapter; application backends and SDK-only
consumers should omit that module. The endpoint-free runtime lives in
`quantum-seed-core`.

The endpoints are disabled to non-admin users via role checks.
...
- GET /admin/seeds/verify/{realm}
  - Verifies all applicable seed packs for the realm.

- GET /admin/seeds/{realm}/{seedPack}/verify
  - Verifies one seed pack for the realm.

- GET /admin/seeds/archetypes/{realm}
  - Lists the seed archetypes applicable to the realm.

- POST /admin/seeds/archetypes/{realm}/{archetypeName}/apply
  - Applies all packs included by the named archetype.
