...
- `MorphiaUtils.VariableBundle` carries both string variables and typed object variables (including Collections and arrays) to the query listener.
- The `QueryToFilterListener` and in-memory `QueryToPredicateJsonListener` support single-variable IN clauses with or without brackets (`field:^${var}` or `field:^[${var}]`), expanding Collections/arrays into native `$in` filters and coercing types (ObjectId, numbers, booleans, dates).

=== Business Scenario: Multi-Tenant Franchise & Dynamic Partner Segmentation
A national retail brand operates diverse franchised store chains sharing a centralized inventory platform. Access is governed not by static roles, but by custom dynamic coordinates: store tiers (`facet.tier: ["FLAGSHIP", "REGIONAL"]`) and sales zones (`coord.salesZone: "NORTHEAST"`).

=== How the Feature Supports the Business Solution
Administrators define a single generic security policy:
[source]
----
tier:^${facet.allowedTiers} && zone:${coord.salesZone} && status:AVAILABLE
----
The `DataDomainResolver` SPI binds the caller's credentials into typed facet collections and zone variables at request time. Bracketless `IN` (`tier:^${facet.allowedTiers}`) automatically unpacks collections into native MongoDB `$in` operators. Unauthenticated or foreign-tenant requests fail closed.
...
- Bracketless single-variable IN for customer visibility or facet tiers:
+
[source]
----
customerId:^${accessibleCustomerIds} && tier:^${facet.allowedTiers}
----
