...
- EdgeDao must be indexed on (tenantId, p, dst) and (tenantId, src, p) for performance.
...
- name: list-orders-by-org
  ...
  effect: ALLOW
  # semantic constraint expressed via hasEdge
  policy:
    hasEdge:
      predicate: placedInOrg
      dstVar: principal.orgRefName
----
...
// Injected once per service when ontology is enabled
EdgeDao edgeDao = /* created with your edges collection */;
...
- Create EdgeDao for your edges collection and call ensureIndexes() on startup.
