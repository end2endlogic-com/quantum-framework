...
note over RuleContext: Expand identities: userId union roles
...
== Row Filter Computation (Data Scoping)

Matching decides _whether_ an action is allowed. Once the effect is `ALLOW`, the
framework derives the row filter that scopes _which records_ a list/query
returns. This is the data-scoping half of enforcement, and it is computed
per-request from the principal, the matched rules, and (optionally) dynamic
access lists.

=== The pipeline

. A `BaseResource` list/query calls `MorphiaRepo.getFilterArray(userFilters, modelClass)`. The caller's own query filters are the starting set; security filters are **AND-ed on top** of them.
. `RepoSecurityFilterBuilder` short-circuits when `SecurityContext.isIgnoringRules()` is set — returning the filters **unscoped**. This `ignoreRules` path is reserved for system-internal/unauthenticated lookups (e.g. credential resolution at login, infrastructure reads). Otherwise it requires both a `PrincipalContext` and a `ResourceContext` and **fails closed** (throws) if either is missing.
. `RuleContext.getFilters(filters, pcontext, rcontext, modelClass)` runs `checkRules`; if the final effect is not `ALLOW` the base filters are returned unchanged. Otherwise it resolves a variable bundle and, for each matched `ALLOW` rule, interpolates the rule's `andFilterString`/`orFilterString` via `MorphiaUtils.convertToFilter(...)`, combining them with the rule's `joinOp` (AND/OR) and de-duplicating.
. The resulting `Filter[]` is AND-ed into the Morphia query.

NOTE: The *realm* is not a row filter — `getRealmId(principal)` returns the
principal's `defaultRealm`, which selects **which database** the query runs
against (realm == database). Row filters scope records _within_ that database.

=== What is scoped by default

There is exactly **one** scope filter baked into the default/system rules:
...
A normal authenticated query therefore gets **tenant isolation only**. Scoping
by organization, account, or data segment is **not automatic** — you must
author rules that reference the corresponding variables (see below).

=== Available substitution variables (static)

`MorphiaUtils.createStandardVariableMapFrom` registers these scalars from the
principal/resource context. They resolve from the principal's `DataDomain` and
`DomainContext`, which are populated from the authenticated token's claims.

[cols="1,3"]
|===
| Variable | Source

| `${pTenantId}`        | `DataDomain.tenantId` (used by the default rule)
| `${pAccountId}`       | `DataDomain.accountNum`
| `${orgRefName}`       | `DataDomain.orgRefName` (note: no `p` prefix)
| `${ownerId}`          | `DataDomain.ownerId`
| `${defaultRealm}`     | `PrincipalContext.defaultRealm` (drives X-Realm scoping)
| `${principalId}`      | `PrincipalContext.userId`
| `${resourceId}`, `${action}`, `${functionalDomain}`, `${area}` | `ResourceContext`
| `${dcTenantId}`, `${dcOrgRefName}`, `${dcAccountId}`, `${dcDataSegment}` | the `DomainContext` (X-Realm-aware; may differ from `DataDomain`)
| `${systemTenantId}`   | `DataDomain.tenantId` (legacy alias)
|===

To scope by org and account in addition to tenant, author a rule whose filter
combines them:

[source,yaml]
----
- name: scope-by-org-and-account
  priority: 500
  securityURI:
    header: { identity: '*', area: '*', functionalDomain: '*', action: 'view' }
    body:   { realm: '*', tenantId: '*', accountNumber: '*', orgRefName: '*' }
  andFilterString: "dataDomain.orgRefName:${orgRefName}"
  orFilterString:  "dataDomain.accountId:${pAccountId}"
  joinOp: AND
  effect: ALLOW
----

IMPORTANT: Because every `${...}` value originates in the token, a token that
omits tenant/org/account claims makes the corresponding filters unresolvable —
even the default `${pTenantId}` filter. A centralized issuer MUST project the
principal's `DomainContext` (realm/tenant/org/account) into token claims for
this machinery to scope correctly.

=== Dynamic access lists (the `AccessListResolver` SPI)

Some scoping is not a static scalar but a *computed collection* — for example,
"the set of location IDs this principal may see". These are supplied by
`AccessListResolver` implementations rather than by the standard variable map.
A resolver whose `key()` returns `accessibleLocationIds` makes the variable
`${accessibleLocationIds}` available, referenced with the in-list operator:
...
The list can be derived however the resolver chooses — a territory assignment,
a `PrincipalContextPropertiesResolver` custom property, or by **walking the
ontology graph**. The ontology path is `ListQueryRewriter` (in
`quantum-ontology-policy-bridge`), which derives accessible IDs from edges such
as `hasEdge("canSeeLocation", ...)`. This is the ontology↔rule seam:
*ontology edges → access list → `${accessibleLocationIds}` → rule filter*.
Predicate references used this way are validated at rule-load time by
`RuleVocabularyValidator`.

Two behaviors distinguish dynamic lists from the static scalars:

- **`supports()` gating.** A resolver is only invoked when its
  `supports(pcontext, rcontext, modelClass)` returns true for the _current
  request's_ context (not the rule's scope). Be permissive, or filter by
  `modelClass`, or the variable will be unresolvable for valid requests.
- **Skip-on-unresolved.** If a rule's filter references a variable that cannot
  be resolved for the current model class (e.g. `${accessibleLocationIds}` on a
  `UserProfile` query), `getFilters` **skips that rule's filter** rather than
  erroring — so a Location access rule does not wrongly constrain unrelated
  queries.
...
[cols="1,1,2"]
|===
| Scope | Kind | How it applies

| Realm        | database selection | `getRealmId` → `defaultRealm` picks the DB
| Tenant       | static, default    | built-in `dataDomain.tenantId:${pTenantId}`
| Org / Account / Segment | static, opt-in | author a rule referencing `${orgRefName}` / `${pAccountId}` / `${dcDataSegment}`
| Location (and similar) | dynamic, resolver-backed | `AccessListResolver` (often ontology-edge-backed) supplies `${accessibleLocationIds}`; requires a rule **and** a registered resolver
|===
...
== Scoped Action Enablement and Dependency Tracking

For the dedicated guide covering the runtime API, manifest model, SPI contracts, and built-in resolver types, see xref:./action-enablement.adoc[Scoped Action Enablement].
...
=== Core idea: the same scoped action tuple stays canonical
...
=== The three answers returned by the framework
...
- `allowed=false` usually means a policy or role problem
- `enabled=false` usually means a feature flag or administrative rollout problem
- `ready=false` usually means setup, configuration, or runtime dependencies are incomplete
...
- `impact`: one of `ALLOWED`, `ENABLED`, or `READY`
- `type`: the dependency type, such as `permission` or `feature-flag`
- `code`: a stable blocker code, such as `permission-denied`
- `message`: a human-readable explanation
- `severity`: typically `warn` or `error`
...
=== Manifest-driven dependency relationships
...
- optional display-oriented metadata such as `displayName` and `description`
...
- action `integration / guided_integration / create` depends on feature flag `guided-integrations`
- action `integration / exchange / create` depends on a tenant filesystem being provisioned
- action `system / some-admin-action / save` depends on a specific config property being present
...
| `permission`
| Evaluate the scoped action through the existing `RuleContext` and permission model
| `ALLOWED`
...
=== Missing manifests and unsupported dependency types

The framework intentionally does not fail open here.

If no manifest entry is registered for a scoped action:

- evaluation falls back to a synthetic requirement that checks `permission`
...
- the response includes `unsupported-dependency-type`
- the blocker is attached with `impact=READY`
...
=== Relationship to policies, feature flags, and UI actions

Think of the layers this way:

1. `@FunctionalMapping` and `@FunctionalAction` define the canonical scoped action tuple in code.
2. Permission rules answer whether that tuple is authorized.
3. Feature flags answer whether the action is turned on for the current cohort or realm.
4. Scoped action requirements answer what else must be configured or provisioned before the action is actually usable.
...
- Use `feature-flag` dependencies for rollout state, not for durable authorization.
- Use `setting-present` and `entity-exists` only for generic prerequisites. Domain-heavy operational checks belong in application or enterprise resolvers.
- Keep blocker codes stable because UIs and automation may key off them.
- Treat capabilities as documentation and UX groupings over scoped actions, not as a substitute for `area / functionalDomain / action`.
...
** Effective roles = JWT roles union `CredentialUserIdPassword.roles`.
...
* Login response model `com.e2eq.framework.model.auth.AuthProvider.LoginPositiveResponse` now includes
  a `roleAssignments` field next to plain `roles`.
* `com.e2eq.framework.model.securityrules.SecurityCheckResponse` also includes `roleAssignments`.

Defaults and compatibility:

* If you do nothing, both login and security check responses automatically derive `roleAssignments`
  from the flat `roles` set and mark each role with `CREDENTIAL`. This preserves backward compatibility.
...
== Field-Level Policy (Property Exclusions)

Row filters (`andFilterString` / `orFilterString`) control *which documents* a
principal can see. Field-level policy controls *which fields of those
documents* — the common "the carrier may see the shipment but not the price"
requirement.

=== Declaring exclusions on a rule

`Rule.excludedFields` carries dotted property paths. The exclusions apply to
any principal/resource the rule matches, composing with the same matching
algorithm as row filters:

[source,java]
----
Rule.Builder()
   .withName("carrier-fulfillment-view")
   .withSecurityURI(uri)                       // who/what this rule matches
   .withEffect(RuleEffect.ALLOW)
   .withAndFilterString("hasEdge('assignedTo', ${principal.orgRefName})")
   .withExcludedFields(List.of("pricing.unitPrice", "pricing.margin"))
   .build();
----

Semantics are *deny-wins*: the union of `excludedFields` across **all**
matched ALLOW rules applies. A field excluded by any applicable rule is
excluded, even if another matched rule does not mention it. Paths that do not
exist on a queried model are ignored (nothing to protect). Excluded fields are
*omitted* from results, not nulled-and-present — clients should treat absence
under an active field policy as "restricted", and the REST layer signals this
with the `X-Field-Policy` response header listing the applied exclusions.

=== Layered enforcement — one policy source, every egress guarded

The resolved exclusion set comes from one place
(`RuleContext.getExcludedFieldPaths`) and is enforced wherever data leaves the
platform. The query language is a multi-datasource contract, so enforcement is
*per egress*, not per database:

[cols="1,2,2"]
|===
|Egress |Mechanism |Notes

|MongoDB reads
|Projection injection in `MorphiaRepo.buildFindOptions`
|Excluded paths never leave the database. Caller `INCLUDE` projections have
policy paths dropped (exclusion wins); caller `EXCLUDE` projections are
unioned with policy paths.

|MongoDB updates
|Preserve-hidden-fields gate in `MorphiaRepo.merge`
|A principal whose policy hides a field cannot blind-overwrite it: the stored
value is restored (via `FieldPolicyEnforcer.copyPath`) before persisting.

|In-memory / hierarchical / custom datasources
|`FieldPolicyEnforcer.mask(result, paths)` at result materialization
|For non-Mongo backends this *is* the datastore-level enforcement.
**Implementing the query contract against a new datasource requires wiring
the enforcer** — it is part of the adapter contract, exactly like honoring
row-filter strings.

|REST serialization
|`FieldPolicyResponseInterceptor` (WriterInterceptor)
|Defense-in-depth fed by the same resolved set: catches computed getters,
reference-expanded fields, and responses assembled outside a secured repo.
Adds the `X-Field-Policy` header when masking occurred.
|===

NOTE: A serializer-only approach (e.g. Jackson `@JsonView`) was considered and
rejected as the primary mechanism: views are compile-time classes while
Quantum policies are tenant-authorable runtime data, and serializer
enforcement guards only the REST door while the reasoner, MCP surface,
exports, and events read through repositories. The serializer layer is kept —
as the last lock, driven by the same rule-resolved path set.

=== Limitations (v1)

* Wildcard paths (`pricing.*`) are not yet supported — list explicit paths.
* The write gate applies on `merge` (partial update); full-document `save` of
  a *new* entity is not field-filtered (the creator authored the values).
* Field exclusions are resolved per (principal, resource) pair, like row
  filters; per-*instance* visibility derives from relationship row filters
  (`hasEdge`), not from per-document field lists.
...
|`${facet.<key>}`
|DataDomainPolicyEntry.facetFilters
|Tenant-configured facet filters (e.g. `${facet.allowedTiers}`)

|`${coord.<key>}`
|DataDomainResolution coordinates
|Custom coordinate variables (e.g. `${coord.salesZone}`)

|`${principal.<attr>}`
|PrincipalContext attributes
|Authenticated principal attributes and claims

|`${dd.<attr>}`
|DataDomain fields
|Target DataDomain partition coordinates
|===

NOTE: **Dotted Variable Identifiers & Single-Variable IN**: Variables with dotted paths (`${facet.tier}`, `${coord.salesZone}`) resolve in a strict hierarchy. When evaluated with the list membership operator (`:^`), single variables resolving to a Java `Collection` (e.g. `allowedTiers = ["FLAGSHIP", "REGIONAL"]`) can be passed without brackets (`tier:^${facet.allowedTiers}`), automatically unpacking into native MongoDB `$in` clauses.
...
=== PrincipalContextPropertiesResolver (SPI) for custom principal properties

[plantuml]
....
@startuml
participant SF as "SecurityFilter"
participant Res as "PrincipalContextPropertiesResolver(s)"
participant PC as "PrincipalContext"
participant MU as "MorphiaUtils"

SF ->> Res: resolve(ResolutionContext)
Res -->> SF: Map<String, Object> props
SF ->> PC: withCustomProperties(props)
note right: Properties cached for request lifetime
...
MU ->> PC: getCustomProperties()
PC -->> MU: customProperties map
note right: Available as ${key} in filter strings
@enduml
....
...
- **Associate/Sales Rep binding**: Link a UserProfile to an Associate entity and expose `associateId` for filtering
- **Territory assignments**: Expose `accessibleTerritoryIds` based on the user's role assignments
- **Organizational hierarchies**: Expose `accessibleLocationIds` via ontology relationships
- **Custom attributes**: Any domain-specific property needed for security filtering

How it works:

- SPI: `com.e2eq.framework.model.securityrules.PrincipalContextPropertiesResolver`
...
- `ResolutionContext` provides access to:
...
- MorphiaUtils.createStandardVariableMapFrom includes custom properties, making them available as `${key}` in filter strings.
...
if (Impersonation header present?) then (yes)
  ...
  if (pass) then (yes)
    ...
    :DataDomain = impersonated user's DomainContext;
  else (no)
    :403;
    stop
  endif
  ...
  if (X-Realm provided and allowed by realmRegEx?) then (yes)
    ...
    :Lookup target Realm's DomainContext;
    :DataDomain = target realm's DomainContext (ownerId = caller);
  else (no)
    :Use caller's default realm;
    :DataDomain = caller's DomainContext;
  endif
endif
...
- With X-Realm (no impersonation):
  ...
  - Permission filter strings can use `${defaultRealm}` to dynamically scope queries based on the active realm.
- With impersonation:
...
- Acting-On-Behalf-Of headers do NOT grant any additional permissions; they are purely for audit purposes.
...
- Integration with Morphia and multi-tenancy: xref:ontology.adoc#ontology-integration-morphia-permissions-section[Integrating Ontology]
...
[NOTE]
====
In `andFilterString` and `orFilterString`, use standard BIAPI syntax:
- 2-argument existence check: `hasEdge(predicate, dst)` and `hasIncomingEdge(predicate, src)`.
- 3-argument qualified check: `hasEdge(predicate, dst, { edgeFilter })` and `hasIncomingEdge(predicate, src, { edgeFilter })`.
- Query macros: `@asOf(timestamp)` for bitemporal validity and `@minConfidence(threshold)` for probabilistic trust.
In `postconditionScript`, use the JavaScript helper counterparts: `hasEdge(predicate, dst, propsOrFn)` and `hasIncomingEdge(predicate, src, propsOrFn)`.
====

=== Business Scenarios & Policy Enforcement Solutions

==== Business Scenario 1: Healthcare Temporary On-Call Staffing & Regulatory Audits
In a hospital network, physicians hold temporary appointments (e.g. "Acting Chief of Emergency Medicine" from Monday 08:00 to Friday 17:00). Policies must guarantee zero-latency expiration without relying on deletion batch cron jobs, while preserving historical audit trails for HIPAA compliance.

*Solution in Permission Rules*:
Using the `@asOf(ts)` macro inside a qualified edge filter ensures that access expires automatically the moment `now >= validTo`, and enables point-in-time compliance audits:
[source,yaml]
----
- name: emergency-department-chart-access
  priority: 100
  securityURI:
    header: { identity: PHYSICIAN, area: clinical, functionalDomain: charts, action: view }
    body:   { realm: '*', orgRefName: '*', accountNumber: '*', tenantId: '*', ownerId: '*', dataSegment: '*', resourceId: '*' }
  effect: ALLOW
  andFilterString: >
...
==== Business Scenario 2: Aerospace Defense Project Classification Guard
In defense environments, the association between an engineering contractor and a classified stealth program (`PROJECT-TITAN`) has a security classification of `SECRET` with an `ITAR` handling compartment. An uncleared sub-contractor querying their project dashboard must not discover that `PROJECT-TITAN` exists or is linked to their team.

*Solution in Permission Rules*:
Using Mandatory Access Control (MAC) edge properties pushed into the compound index scan (`idx_domain_security`), the relationship is withheld at the database level:
[source,yaml]
----
- name: defense-classified-project-view
  priority: 105
  securityURI:
    header: { identity: CONTRACTOR, area: engineering, functionalDomain: projects, action: view }
    body:   { realm: '*', orgRefName: '*', accountNumber: '*', tenantId: '*', ownerId: '*', dataSegment: '*', resourceId: '*' }
  effect: ALLOW
  andFilterString: >
...
- MongoDB compound indexes `idx_domain_p_dst_props`, `idx_domain_temporal`, `idx_domain_security`, and `idx_domain_confidence` accelerate edge lookups directly at the datastore layer.
...
[source,javascript]
----
// Permit view if the current resource is a location reachable from the caller
