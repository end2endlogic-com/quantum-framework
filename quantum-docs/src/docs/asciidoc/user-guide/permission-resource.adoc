...
The `/system/permissions/check` endpoint now accepts an optional `evalMode` parameter to control
how the server evaluates filter strings and postcondition scripts in single-resource checks.
