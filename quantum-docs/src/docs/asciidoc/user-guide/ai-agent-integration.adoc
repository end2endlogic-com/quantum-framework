...
| `/api/agent/query-hints` | GET | Query grammar summary, example queries by intent, and "did you know" hints (expand, ontology). Use so agents can answer "what query gets locations in Atlanta?" and suggest expand/ontology usage. |
| `/api/agent/permission-hints` | GET | Permission check/evaluate API summary, area/domain/action mapping (e.g. Query Gateway), example check requests, and "did you know" hints. Use so agents can answer "can role X perform action Y on entity Z?" and "if not, why not?" (winningRuleName), suggest rule changes, and generate least-privilege rules for a scenario. |
| `/api/agent/ontology` | GET | Optional: ontology summary for retrieval context. |
| `/api/agent/execute` | POST | Execute one of the gateway tools by name + arguments (delegate only to `QueryGatewayResource`). |
