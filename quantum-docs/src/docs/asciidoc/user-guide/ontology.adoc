...
When your application saves or updates entities, collect the explicit edges implied by the entity’s fields and call OntologyMaterializer to compute and persist inferred edges. The reasoner will apply traits discovered by the loader (inverse, symmetric, transitive, subPropertyOf) automatically.

[source,java]
----
@Inject OntologyMaterializer materializer;

public void onOrderSaved(String tenant, Order order) {
  List<Reasoner.Edge> explicit = new ArrayList<>();
  if (order.getPlacedBy() != null) {
    explicit.add(new Reasoner.Edge(order.getRefName(), "placedBy", order.getPlacedBy().getRefName(), false, Optional.empty()));
  }
  if (order.getInOrgDirect() != null) {
    explicit.add(new Reasoner.Edge(order.getRefName(), "placedInOrg", order.getInOrgDirect().getRefName(), false, Optional.empty()));
  }
  materializer.apply(tenant, order.getRefName(), "Order", explicit);
}
----

In many cases you only need to pass the minimal explicit edges (e.g., placedBy and memberOf via customer); property chains configured elsewhere or declared in code will infer placedInOrg as before.
