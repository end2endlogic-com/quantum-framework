...
the historical `DEDICATED_REALM` behavior.
...
- skips realm migrations and index initialization
...
TTL indexes must be installed explicitly by realm migrations for every
tenant-scoped collection. Large collections may be stamped in resumable
batches, but every batch uses the same purge reference and timestamp. Blob
payloads owned by tenant rows are part of both archive and expiration coverage.
