...
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  echo "This script must be run inside a git repository (cd to the repository root or to quantum-docs)." >&2
  exit 1
fi

# Move to repo root if run from quantum-docs/
...
echo "  mvn -pl quantum-docs -am -DskipTests deploy"
