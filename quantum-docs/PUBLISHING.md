...
**1.4.1-SNAPSHOT** (in progress) is published only under **`gh-pages/1.4.1-SNAPSHOT/`**.

`maven-scm-publish-plugin` uses **`ignorePathsToDelete`** so that when LTS is pushed to the root, the **`1.4.1-SNAPSHOT/`** directory is not removed.
...
| `docs-ghpages-1.4` | `docs.ghpages.subdir` = `1.4.1-SNAPSHOT` → publish preview under that folder only. |
...
- For **correct 1.4 preview text**: build on **`1.4.1-SNAPSHOT`**, then `-Pdocs-ghpages-1.4`.

## URLs

- **LTS (default):** `https://<org>.github.io/<repo>/` (and paths under it, e.g. `user-guide/index.html`).
- **1.4 preview:** `https://<org>.github.io/<repo>/1.4.1-SNAPSHOT/…`
...
Publish **1.4.1-SNAPSHOT** preview:
